# services/core_project/app/billing_app/views.py

from io import BytesIO

from django.http import HttpResponse
from django.shortcuts import render
from django.contrib.auth.decorators import login_required
from django.views import View
from rest_framework import viewsets, status
from rest_framework.decorators import action
from rest_framework.response import Response
from reportlab.pdfgen import canvas

from .models import UsageRecord, Invoice
from .serializers import UsageRecordSerializer, InvoiceSerializer


class UsageViewSet(viewsets.ModelViewSet):
    queryset = UsageRecord.objects.all().order_by('-timestamp')
    serializer_class = UsageRecordSerializer


class InvoiceViewSet(viewsets.ViewSet):
    @action(detail=False, methods=['post'])
    def generate(self, request):
        user = request.user
        start = request.data.get('start')
        end = request.data.get('end')

        qs = UsageRecord.objects.filter(
            user=user,
            timestamp__date__gte=start,
            timestamp__date__lte=end
        )
        total = qs.count()
        amount = total * 0.10  # e.g. $0.10 per call

        inv = Invoice.objects.create(
            user=user,
            period_start=start,
            period_end=end,
            total_calls=total,
            amount_due=amount
        )

        # generate PDF invoice
        buffer = BytesIO()
        p = canvas.Canvas(buffer)
        p.drawString(100, 800, f"Invoice for {user.username}")
        p.drawString(100, 780, f"Period: {start} to {end}")
        p.drawString(100, 760, f"Calls: {total}")
        p.drawString(100, 740, f"Amount due: ${amount:.2f}")
        p.showPage()
        p.save()
        buffer.seek(0)
        pdf_data = buffer.read()

        return Response(
            {'invoice_id': inv.id, 'pdf': pdf_data},
            status=status.HTTP_201_CREATED
        )


class InvoicePDFView(View):
    def get(self, request, pk):
        try:
            invoice = Invoice.objects.get(pk=pk)
        except Invoice.DoesNotExist:
            return HttpResponse("Invoice not found", status=404)

        buffer = BytesIO()
        p = canvas.Canvas(buffer)
        p.drawString(100, 800, f"Invoice for {invoice.user.username}")
        p.drawString(100, 780, f"Period: {invoice.period_start} to {invoice.period_end}")
        p.drawString(100, 760, f"Calls: {invoice.total_calls}")
        p.drawString(100, 740, f"Amount due: ${invoice.amount_due:.2f}")
        p.showPage()
        p.save()
        buffer.seek(0)

        response = HttpResponse(buffer, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="invoice_{pk}.pdf"'
        return response


@login_required
def dashboard(request):
    return render(request, 'billing/dashboard.html')
